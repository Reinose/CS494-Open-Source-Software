...
db = pymysql.connect(host='172.17.0.2', port=3306, user='reinose', passwd='password', db='cs494')
cursor = db.cursor()
cursor.execute("USE cs494")

@app.route('/')
def hello_world():
    return cursor.execute("select * from testset limit 1");
